...
本文档描述了web服务器和Perl web 应用与框架直接的标准接口规范。
设计该接口的目的是推动web应用的可移植性并减少web应用框架作者们的重复工作。
...
请记住PSGI并不是另一个web应用框架。PSGI是一套为了分离web服务器环境和web应用框架代码而制定的规范。
PSGI也不是web应用API。Web应用的开发者们（末端用户）不需要将他们的应用直接运行在PSGI接口上，
而是被鼓励使用支持PSGI的框架。
