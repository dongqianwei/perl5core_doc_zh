...
当你做这件事的时候，你通过创建C<OP_CUSTOM>类型的ops和你自己的PP函数的C<op_ppaddr>将普通的Perl ops替换为定制的ops。
着应该被定义在XS代码中，应该看上去向C<pp_*.c>中的PP ops。你要保证你的op会从栈上获取合适数量的值，你需要在必要的时候添加栈标记。
...
你应该向Perl解释器"注册"你的op，这样它才能产生有意以的错误和警告信息。
因为"逻辑上的"op类型C<OP_CUSTOM>可能有多个定制op，Perl使用C<< o->op_ppaddr >>来区分使用哪一个定制op来处理它。
你应该为每一个ppaddr创建C<XOP>结构，用C<XopENTRY_set>为定制op设置属性，使用C<Perl_custom_op_register>注册ppaddr对应的结构。
一个简单的例子：
...
你的op的名称。这将会被包含中错误信息中，可以被L<B|B>模块作为C<< $op->name >>返回，
因此它会出现在诸如L<B::Concise|B::Concise>这样的模块的输出中。
...
op使用的C<*OP>结构类型。这个应该是F<op.h>中定义的C<OA_*>常量，如：
...
它应该被解释为'C<PVOP>'。C<_OR_SVOP>是因为C<PVOP>, C<OP_TRANS>,有时会被C<SVOP>取代。
...
这个成员是C<Perl_cpeep_t>类型的，它将会拓展为C<void(*Perl_cpeep_t)(aTHX_ OP *o, OP *oldop)>。
如果它被设置了，这个函数将会在窥孔优化器遇到这种类型的ops时从C<Perl_rpeep>调用。
I<o>是需要优化的OP；I<oldop>是之前被优化的OP，其C<op_next>字段指向I<o>。

=back

C<B::Generate> directly supports the creation of custom ops by name.

C<B::Generate>直接支持通过名称创建定制op。
...
自1997年5月，本文档由Jeff Okamoto E<lt>okamoto@corp.hp.comE<gt>维护。
它现在是作为Perl的一部分由Perl 5 Porters E<lt>perl5-porters@perl.orgE<gt>维护。
...
大量的帮助和建议来自于 Dean Roehrich, Malcolm Beattie,
Andreas Koenig, Paul Hudson, Ilya Zakharevich, Paul Marquess, Neil
Bowers, Matthew Green, Tim Bunce, Spider Boardman, Ulrich Pfeifer,
Stephen McCamant, and Gurusamy Sarathy.
