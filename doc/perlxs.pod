...
XS接口与原有的库被联合起来创建一个新库以被perl动态加载或被静态链接到perl中。
...
每一个XSUB对应一个C函数定义，它提供了粘合Perl调用约定域C调用约定的胶水。
...
这些胶水代码将参数从Perl栈上拉出来，将这些Perl的值装为C函数期待的形式，
调用C函数，将C函数返回的结果返回给Perl。
这里的返回值也许是一个常规的C返回值或者任何作为出参的C函数参数。
这些返回值可能通过放置到Perl栈的方式或者修改Perl提供的参数的方式被传回。
...
以上内容就是事实发生的事情的简化描述。由于Perl允许比C更加灵活的调用约定，
XSUBs也许会比实际上做更多的工作，比如检查输入参数的合法性，
...
当然，也可以直接用C写胶水代码。但是，这将会事非常麻烦的任务，
尤其是当你需要为多个C函数写胶水代码，或者你对Perl栈的特点诸如此类的的东西不熟悉的话。
XS将为你解决这些问题：你可以写一段更加精确简洁的关于胶水代码应该做什么的I<描述>，
...
XS语言允许你描述C代码是如何使用的以及对应的Perl代码是如何使用的之间的映射关系。
它同样允许创建会被直接被翻译为C代码Perl例程，即使它和已存在的C函数没有关系。
在C接口和Perl接口相似的场景下，XSUB声明和C函数的声明（K&R形式）基本一致。
在这样的场景下，有另一个工具C<h2xs>可以将整个C头文件翻译为对应的XS文件，
其将提供头文件中描述的函数/宏的胶水代码。
